...
// Génère un nom de champ POST stable pour éviter les problèmes d'espaces/accents
function getVoteInputName($position) {
...
// Récupérer les informations de l'utilisateur avant le traitement du vote
...
// Définir les variables pour l'affichage et les emails
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_vote'])) {
    ...
    try {
        ...
        foreach ($valid_positions as $position) {
            ...
            $input_name = getVoteInputName($position);
        ...
        $email_sent = sendVoteConfirmationEmail($user_info['email'] ?? '', $full_name, $election['title']);
        ...
        // Notifier l'administrateur qu'un vote a été enregistré
        $admin_notified = sendAdminVoteNotificationEmail(
            $full_name,
            $user_info['email'] ?? '',
            $election['title'],
            $positions_to_vote,
        ...
        if ($admin_notified) {
            error_log("Email de notification admin envoyé pour l'utilisateur ID: " . $_SESSION['user_id']);
        } else {
            error_log("Échec de l'envoi de la notification admin pour l'utilisateur ID: " . $_SESSION['user_id']);
        }
                                            ...
                                            <div class="space-y-4">
                                                <?php foreach ($candidates_by_position[$position] as $candidate): ?>
                                                    <label class="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg hover:border-blue-400 cursor-pointer">
                                                        <input type="radio" name="<?php echo htmlspecialchars(getVoteInputName($position)); ?>" 
                            ...
                            <?php foreach ($results as $position => $position_results): ?>
                                <div class="mb-8">
                                    ...
                                    <div class="bg-white rounded-lg shadow-sm p-4">
                                        <?php
                                            // Filtrer les résultats pour ne garder que les candidats réels (pas les votes blancs)
                                            $real_candidates = array_filter($position_results, function($r) {
                                                return $r['name'] !== 'Votes blancs';
                                            });
                                            
                                            // Déterminer le vainqueur parmi les candidats réels uniquement
                                            $winner = !empty($real_candidates) ? reset($real_candidates) : null;
                                            $winner_candidate = null;
                                            $has_tie = false;
                                            
                                            // Vérifier s'il y a une égalité entre les deux premiers candidats réels
                                            if ($winner && count($real_candidates) > 1) {
                                                $real_candidates_list = array_values($real_candidates);
                                                if ($real_candidates_list[0]['votes'] === $real_candidates_list[1]['votes']) {
                                                    $has_tie = true;
                                                }
                                            }
                                        ...
                                        <?php elseif ($winner): ?>
                                            <div class="text-center py-4 text-gray-500">
                                                ...
                                                Les votes blancs ont remporté cette élection avec <?php echo $winner['percentage']; ?>% des votes. Le vainqueur parmi les candidats réels est <strong><?php 
                                                    if (!empty($real_candidates)) {
                                                        $first_real = reset($real_candidates);
                                                        echo htmlspecialchars($first_real['name']);
                                                    } 
                                                ?></strong> avec <?php echo (!empty($real_candidates) ? reset($real_candidates)['votes'] : 0); ?> vote<?php echo (!empty($real_candidates) && reset($real_candidates)['votes'] > 1 ? 's' : ''); ?>.
                                            </div>
                                        <?php endif; ?>
