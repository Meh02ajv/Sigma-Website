...
// Vérifier immédiatement la session admin avant toute opération DB lourde
if (
    !isset($_SESSION['admin_logged_in']) ||
    $_SESSION['admin_logged_in'] !== true ||
    !isset($_SESSION['admin_email']) ||
    ADMIN_EMAIL === '' ||
    !hash_equals(ADMIN_EMAIL, (string)$_SESSION['admin_email'])
) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_general_config'])) {
    ...
    // Upload vidéo hero - VERSION SIMPLIFIé‰E ET ROBUSTE
    if (isset($_FILES['hero_video']) && $_FILES['hero_video']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            $upload_folder = __DIR__ . '/uploads/videos/';
            ...
            if ($success && file_exists($destination)) {
                ...
                $_SESSION['error'] = "Échec de l'upload. Vérifiez que le dossier uploads/videos/ existe et a les bonnes permissions.";
            }
        }
    }
    ...
    if (isset($_POST['delete_hero_video'])) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $local_video_path = __DIR__ . '/' . ltrim(str_replace('\\', '/', $video_path), '/');
            if (file_exists($video_path)) {
                @unlink($video_path);
            } elseif (file_exists($local_video_path)) {
...
$default_config = [
    ...
    'contact_address' => '123 Rue de l\'é‰ducation, 75001 Paris, France',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_event'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "é‰vénement ajouté avec succès.";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_event'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "é‰vénement supprimé avec succès.";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_event'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "é‰vénement mis à jour avec succès.";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_election'])) {
    ...
    if (!$vote_start_dt || !$end_date_dt || !$results_date_dt) {
        ...
        if ($stmt->execute()) {
            $_SESSION['success'] = "é‰lection créée avec succès.";
            // DEBUG: Log de succès
            error_log("é‰lection créée avec succès: " . $title);
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_election'])) {
    ...
    $stmt->bind_param("i", $election_id);
    if ($stmt->execute()) {
        $_SESSION['success'] = "é‰lection supprimée avec succès.";
    } else {
...
<head>
    ...
    <style>
        ...
        /* Responsive - Tablettes et petits écrans (1024px et moins) */
        ...
        /* Responsive - Tablettes et petits écrans (768px et moins) */
        ...
        /* Responsive - Très petits écrans et mobiles (480px et moins) */
        ...
        /* Responsive - Ultra petits écrans (moins de 360px) */
        ...
        /* Onglets - Style et responsivité */
        .tabs {
            display: flex;
            border-bottom: 2px solid var(--sigma-border);
            margin-bottom: 1.5rem;
            overflow-x: auto;
            overflow-y: hidden;
        ...
        /* Onglets responsive - Tablettes (768px et moins) */
        @media (max-width: 768px) {
            .tabs {
        ...
        /* Onglets responsive - Mobiles (480px et moins) */
        @media (max-width: 480px) {
            .tabs {
                margin-bottom: 1rem;
                border-bottom: 1px solid var(--sigma-border);
        ...
        /* Onglets responsive - Ultra petits écrans (360px et moins) */
        @media (max-width: 360px) {
            .tab {
                padding: 0.5rem 0.875rem;
                font-size: 0.75rem;
        ...
        input[type="file"] {
            ...
            width: 100%;
            padding: 0.75rem;
            border: 1px dashed var(--sigma-border);
            border-radius: 8px;
            cursor: pointer;
            background-color: #f8fafc;
            transition: all 0.2s ease;
        ...
        /* Responsivité Formulaires - Tablettes (768px et moins) */
        @media (max-width: 768px) {\n            select,\n            textarea {\n                font-size: 0.95rem;\n                padding: 0.65rem 0.875rem;\n            }\n            \n            textarea {\n                min-height: 100px;\n            }\n            \n            input[type=\"file\"] {\n                padding: 0.65rem;\n            }\n            \n            input[type=\"checkbox\"],\n            input[type=\"radio\"] {\n                width: 16px;\n                height: 16px;\n            }\n        }\n        \n        /* Responsivité Formulaires - Mobiles (480px et moins) */\n        @media (max-width: 480px) {\n            select,\n            textarea {\n                font-size: 0.9rem;\n                padding: 0.6rem 0.75rem;\n            }\n            \n            textarea {\n                min-height: 90px;\n            }\n            \n            input[type=\"file\"] {\n                padding: 0.5rem;\n                font-size: 0.85rem;\n            }\n            \n            input[type=\"checkbox\"],\n            input[type=\"radio\"] {\n                width: 16px;\n                height: 16px;\n            }\n        }\n        \n        /* Responsivité Formulaires - Ultra petits écrans (360px et moins) */\n        @media (max-width: 360px) {\n            select,\n            textarea {\n                font-size: 0.85rem;\n                padding: 0.55rem 0.65rem;\n            }\n            \n            textarea {\n                min-height: 80px;\n            }\n            \n            input[type=\"file\"] {\n                padding: 0.45rem;\n                font-size: 0.8rem;\n            }\n        }\n        \n        /* Conteneurs de grille wrap responsive */\n        .form-row {\n            display: flex;\n            gap: 1rem;\n            flex-wrap: wrap;\n        }\n        \n        .form-row > * {\n            flex: 1;\n            min-width: 200px;\n        }\n        \n        @media (max-width: 768px) {\n            .form-row {\n                gap: 0.75rem;\n            }\n            \n            .form-row > * {\n                min-width: 100%;\n                flex: 1 1 100%;\n            }\n        }\n        \n        /* Texte et contenu responsive */\n        .admin-container > * {\n            word-break: break-word;\n            overflow-wrap: break-word;\n        }\n    </style>
</head>
<body>
    <div class="admin-container">
        ...
        <div class="main-content">
            ...
            <section id="events-management" class="section">
                <div class="card">
                    <h2 class="text-2xl font-bold mb-6"><i class="fas fa-calendar-alt text-blue-600 mr-2"></i>Gestion des é‰vénements</h2>
                    ...
                    <!-- Existing Events -->
                    <h3 class="text-xl font-semibold mb-4">é‰vénements existants</h3>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead>
                                <tr class="bg-gray-100">
                                ...
                                <?php foreach ($events as $event): ?>
                                    <tr class="border-b">
                                        ...
                                        <td class="p-3">
                                            ...
                                            if ($eventDate > $now) {
                                                echo '<span class="text-green-600">é€ venir</span>';
                                            } else {
                            ...
                            <div class="mb-4 p-4 border rounded-lg">
                                ...
                                <div class="mb-2">
                                    <?php if ($general_config['hero_video'] && file_exists($general_config['hero_video'])): ?>
                                        ...
                                        <button type="submit" name="delete_hero_video" class="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600 mt-2" onclick="return confirm('éŠtes-vous sÛr de vouloir supprimer cette vidéo ?');">
                ...
                <div class="card">
                    ...
                    <div class="grid md:grid-cols-3 gap-6">
                        ...
                        <div class="theme-card border-2 rounded-lg p-6 cursor-pointer hover:shadow-lg transition" data-theme="independence">
                            <div class="text-center mb-4">
                                <div class="text-5xl mb-3">ðŸ‡¹ðŸ‡¬</div>
                    ...
                    <form method="POST" class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" id="election_title" name="election_title" placeholder="Ex: é‰lection du bureau 2025" class="form-input" required>
                            </div>
                            <div>
            ...
            <section id="results" class="section">
                <div class="card">
                    <h2 class="text-2xl font-bold mb-6"><i class="fas fa-chart-bar text-blue-600 mr-2"></i>Résultats et Suivi des é‰lections</h2>
                                ...
                                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg mb-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center mb-4">
                                        <div class="bg-white p-4 rounded-lg shadow">
                                            ...
                                            <div class="text-sm text-gray-600 mt-1">é‰lecteurs inscrits</div>
                                        </div>
                                        <div class="bg-white p-4 rounded-lg shadow">
                                                            ...
                                                            <div class="flex items-center justify-between p-3 rounded-lg <?php echo $index === 0 && !$result['is_blank'] ? 'bg-green-50 border-2 border-green-200' : 'bg-gray-50'; ?>">
                                                                <div class="flex items-center space-x-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <?php if ($index === 0 && !$result['is_blank']): ?>
                                                                            <span class="ml-2 bg-green-600 text-white px-2 py-0.5 rounded text-xs font-medium">é‰lu(e)</span>
                                                                        <?php endif; ?>
                                                                    </div>
                                                                </div>
                            ...
                            <?php else: ?>
                                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <p class="font-medium text-yellow-800">é‰lection en cours</p>
    ...
    <div class="modal" id="editUserModal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label class="form-label">é‰tudes</label>
