...
// Load PHPMailer if available (optional - form will work without it)
...
// Validate CSRF token (comparaison constante pour limiter les faux négatifs)
...
// Connect to the database using config.php variables
try {
...
try {
    ...
    // Send notification email to admin (only if PHPMailer is available)
    ...
    try {
        ...
        $admin_mail->SMTPSecure = \PHPMailer\PHPMailer\PHPMailer::ENCRYPTION_STARTTLS;
        $admin_mail->Port = SMTP_PORT;
        $admin_mail->CharSet = 'UTF-8';

        // Recipients
        $admin_email_addr = (defined('ADMIN_NOTIFICATION_EMAIL') && ADMIN_NOTIFICATION_EMAIL !== '') ? ADMIN_NOTIFICATION_EMAIL : 'gojomeh137@gmail.com';
        $admin_mail->addAddress($admin_email_addr, 'Administrateur');
