...
<section class="hero">
    ...
    $hero_video = 'uploads/videos/hero_background.mp4'; // Valeur par défaut prioritaire
    ...
    if ($stmt) {
        ...
        if ($row = $result->fetch_assoc()) {
            $stored_hero_video = trim((string)$row['setting_value']);
            $stored_hero_video = str_replace('\\', '/', $stored_hero_video);

            if ($stored_hero_video !== '') {
                if (preg_match('~uploads/videos/[^/]+$~i', $stored_hero_video, $matches)) {
                    $hero_video = $matches[0];
                } elseif (preg_match('~^(?:[A-Za-z]:/|/)~', $stored_hero_video)) {
                    $hero_video = 'uploads/videos/' . basename($stored_hero_video);
                } else {
                    $hero_video = ltrim($stored_hero_video, '/');
                }
            }
        }
        $stmt->close();
    ...
    $hero_video_candidates = [
        $hero_video,
        'uploads/videos/hero_background.mp4',
        'img/hero-video.mp4'
    ];

    $resolved_hero_video = 'img/hero-video.mp4';
    foreach ($hero_video_candidates as $candidate) {
        $candidate_path = __DIR__ . '/' . ltrim($candidate, '/');
        if (file_exists($candidate_path)) {
            $resolved_hero_video = $candidate;
            break;
        }
    }

    $hero_video = $resolved_hero_video;
    ...
    <div class="hero-content">
        ...
        <p>Reconnectez-vous avec vos anciens camarades, découvrez les évènements à venir et contribuez à notre communauté dynamique.</p>
