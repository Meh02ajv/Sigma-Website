...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT email, full_name, is_admin FROM users WHERE email = ?");
    ...
    if ($user['is_admin'] == 1 && defined('ADMIN_NOTIFICATION_EMAIL') && ADMIN_NOTIFICATION_EMAIL !== '') {
        $recipient_email = ADMIN_NOTIFICATION_EMAIL;
    }
    
    if (sendEmail($recipient_email, $full_name, $subject, $body, $altBody)) {
