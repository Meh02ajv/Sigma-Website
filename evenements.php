                    ...
                    <div class="event-card">
                        ...
                        <div class="event-details">
                            ...
                            <?php if (isset($_SESSION['user_id'])): ?>
                                ...
                                $reminder_stmt = $conn->prepare($reminder_check_sql);

                                if ($reminder_stmt) {
                                    $reminder_stmt->bind_param("ii", $_SESSION['user_id'], $event['id']);
                                    $reminder_stmt->execute();
                                    $reminder_exists = $reminder_stmt->get_result()->num_rows > 0;
                                    $reminder_stmt->close();
                                }
                                ?>
                                <div class="reminder-section">
                                    <?php if ($reminder_exists): ?>
                                        <button class="btn-reminder btn-reminder-added" data-event-id="<?php echo (int)$event['id']; ?>" disabled>
                                            <i class="fas fa-bell"></i> Rappel ajouté
                                        </button>
                                    <?php else: ?>
                                        <button class="btn-reminder" data-event-id="<?php echo (int)$event['id']; ?>">
                                            <i class="far fa-bell"></i> Ajouter un rappel
                                        </button>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
