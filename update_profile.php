...
$return_page = $_POST['return_page'] ?? ($_GET['from'] ?? ($_SESSION['profile_return_page'] ?? 'settings.php'));
if (!in_array($return_page, $allowed_pages, true)) {
    $return_page = 'settings.php';
}
$_SESSION['profile_return_page'] = $return_page;

function redirectToProfile($returnPage, $extraQuery = '') {
    $location = 'mod_prof.php?from=' . rawurlencode($returnPage);
    if ($extraQuery !== '') {
        $location .= '&' . $extraQuery;
    }
...
// (config.php already creates $conn with the correct credentials)
...
if ($password && strlen($password) < 8) {
    ...
    redirectToProfile($return_page, 'reset=1');
}
