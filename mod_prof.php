...
// Generate CSRF token once per session lifecycle (evite les erreurs inter-onglets)
...
$return_page = isset($_GET['from']) ? $_GET['from'] : ($_SESSION['profile_return_page'] ?? 'settings.php');
...
$_SESSION['profile_return_page'] = $return_page;
...
if (strpos($user_email, '@') !== false) {
    [$local_part, $domain_part] = explode('@', $user_email, 2);
    $visible_prefix = substr($local_part, 0, 2);
    if ($visible_prefix === false) {
        $visible_prefix = '';
    }
    $masked_email = $visible_prefix . str_repeat('*', max(0, strlen($local_part) - strlen($visible_prefix))) . '@' . $domain_part;
}
?>
...
<body>
            ...
            <form id="profileForm" method="POST" action="update_profile.php" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <div class="label-with-action">
                        ...
                        <button type="button" class="toggle-email" id="toggleEmailBtn" onclick="toggleEmail()" aria-controls="email" aria-pressed="false">Afficher</button>
                    </div>
                    <div class="input-icon">
                        <i class="fas fa-envelope"></i>
                        <input type="text" id="email" name="email" class="form-control" value="<?php echo htmlspecialchars($masked_email); ?>" data-full-email="<?php echo htmlspecialchars($user_email); ?>" data-masked-email="<?php echo htmlspecialchars($masked_email); ?>" readonly required>
                    </div>
                </div>
    ...
    <script>
        ...
        function toggleEmail() {
            ...
            const toggleButton = document.getElementById('toggleEmailBtn');
            const fullEmail = emailInput.dataset.fullEmail || '';
            const maskedEmail = emailInput.dataset.maskedEmail || fullEmail;
