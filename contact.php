...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_contact'])) {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        $_SESSION['contact_error'] = "Erreur de validation CSRF.";
    ...
    if (empty($name) || empty($email) || empty($subject) || empty($message) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $_SESSION['contact_error'] = "Veuillez remplir tous les champs correctement.";
        header("Location: contact.php");
        exit;
    }
    ...
    if ($success) {
        ...
        if (sendEmail($contact_info['email'], 'SIGMA Alumni', $email_subject, $email_body, $email_altBody)) {
            ...
            $_SESSION['contact_error'] = "Erreur lors de l'envoi de l'email.";
        }
    } else {
...
<body>
    ...
    <div class="contact-container">
        <?php if (isset($_SESSION['contact_success'])): ?>
            <div class="success-message"><?php echo htmlspecialchars($_SESSION['contact_success'], ENT_QUOTES, 'UTF-8'); unset($_SESSION['contact_success']); ?></div>
        <?php endif; ?>
        <?php if (isset($_SESSION['contact_error'])): ?>
            <div class="error-message"><?php echo htmlspecialchars($_SESSION['contact_error'], ENT_QUOTES, 'UTF-8'); unset($_SESSION['contact_error']); ?></div>
        <?php endif; ?>
